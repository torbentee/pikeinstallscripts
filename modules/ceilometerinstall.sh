...
then
	...
	# Patch.... packaged gnochi-api.conf wsgi contains errors
	cat ./libs/gnocchi/wsgi-gnocchi.conf > /etc/apache2/sites-available/gnocchi-api.conf
	cat ./libs/gnocchi/wsgi-gnocchi.conf > /etc/apache2/sites-enabled/gnocchi-api.conf
	sed -r -i "s/Listen\ 8041/Listen\ $gnocchihost:8041/g" /etc/apache2/sites-enabled/gnocchi-api.conf
	sed -r -i "s/Listen\ 8041/Listen\ $gnocchihost:8041/g" /etc/apache2/sites-available/gnocchi-api.conf
	systemctl stop apache2
	sleep 5
	systemctl start apache2
	sleep 5
